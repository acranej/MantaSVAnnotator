...
#' @return SV data table with columns added indicating if the SV breakend occurs in a gene and the name of the gene
...
#' @export
annotate_exon = function(i, bedpe_input) {
  bedpe <- bedpe_input[i,]
  ### Annotate break point A by giving exons that exist where the breakpoint occurs
  temp_A_gr <- GRanges(bedpe$CHROM_A[1], 
                       IRanges(bedpe$START_A[1], bedpe$END_A[1]))
  temp_A_gr_ant <-  suppressWarnings(exon_locations %&% temp_A_gr)
  temp_A_dt_ant <- as.data.table(temp_A_gr_ant)
  ### collapse all exons that occur in breakpoint A
  if(!nrow(temp_A_dt_ant) == 0) {
    if(nrow(temp_A_dt_ant) == 1) {
      bedpe$BREAK_A_Ensembl_Exon[1] <- temp_A_dt_ant$Exon_Ensembl_ID[1]
    } else {
      bedpe$BREAK_A_Ensembl_Exon[1] <- paste0(temp_A_dt_ant$Exon_Ensembl_ID, collapse = ";")
    }
  } else {
    bedpe$BREAK_A_Ensembl_Exon[1] <- ""
  }
  ### Annotate break point B
  temp_B_gr <- GRanges(bedpe$CHROM_B[1], 
                       IRanges(bedpe$START_B[1], bedpe$END_B[1]))
  temp_B_gr_ant <-  suppressWarnings(exon_locations %&% temp_B_gr)
  temp_B_dt_ant <- as.data.table(temp_B_gr_ant)
  ### collapse all exons that occur in breakpoint B 
  if(!nrow(temp_B_dt_ant) == 0) {
    if(nrow(temp_B_dt_ant) == 1) {
      bedpe$BREAK_B_Ensembl_Exon[1] <- temp_B_dt_ant$Exon_Ensembl_ID[1]
    } else {
      bedpe$BREAK_B_Ensembl_Exon[1] <- paste0(temp_B_dt_ant$Exon_Ensembl_ID, collapse = ";")
    }
  } else {
    bedpe$BREAK_B_Ensembl_Exon[1] <- ""
  }
  return(bedpe)
}
...
option_list <- list(
  ...
  make_option(c("-e", "--exonannot"), type = "character", default = NULL, help = "Path the exon annotation file"),
